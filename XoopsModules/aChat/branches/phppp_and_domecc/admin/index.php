...
function purge2() {
	...
	global $HTTP_POST_VARS, $msgobj_h;
	...
	$daynumbers = intval(@$_POST['daynumbers']);
...
function purge_create_log($n) {
	...
	$result = $xoopsDB -> query( "SELECT `date` FROM " . $xoopsDB -> prefix( "achat_messages" ) . " ORDER BY `date` ASC, `mid` ASC", 1 );
	list( $startdate ) = $xoopsDB->fetchRow( $result );
	$result = $xoopsDB -> query( "SELECT `date` FROM " . $xoopsDB -> prefix( "achat_messages" ) . " ORDER BY `date` ASC, `mid` ASC", 1, intval($n)-1 );
